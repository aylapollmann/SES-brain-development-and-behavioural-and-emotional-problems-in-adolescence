...
```{r, include = FALSE}
setwd("~/Library/CloudStorage/OneDrive-King'sCollegeLondon/Project 4/Code/WKS")
#save.image(file = "4B. TEST- Kmeans.RData")
#load(file = "4B. TEST- Kmeans.RData")
```

```{r, include = FALSE}
setwd("~/Library/CloudStorage/OneDrive-King'sCollegeLondon/3. Shared Documents Lab/Data/ABCD/ABCD Ayla/Project 4")
DataImp <- read_excel("Test_DataImp.xlsx")
Data <- read_excel("Test_Data.xlsx")
```

```{r}
#Number of participants per measure
CountB1 <- length(Data$b1[!is.na(Data$b1)])
CountB2 <- length(Data$b2[!is.na(Data$b2)])
CountB3 <- length(Data$b3[!is.na(Data$b3)])

CountIn1 <- length(Data$In1[!is.na(Data$In1)])
CountIn2 <- length(Data$In2[!is.na(Data$In2)])
CountIn3 <- length(Data$In3[!is.na(Data$In3)])

CountM1 <- length(Data$m1[!is.na(Data$m1)])
CountM2 <- length(Data$m2[!is.na(Data$m2)])
CountM3 <- length(Data$m3[!is.na(Data$m3)])

OverallCount <- list(CountB1, CountB2, CountB3, CountIn1, CountIn2, CountIn3, CountM1, CountM2, CountM3)
...
```{r}
DataImp$AgeDistance1 <- (DataImp$AgeT2 - DataImp$AgeT1)
DataImp$AgeDistance2 <- (DataImp$AgeT3 - DataImp$AgeT2)

D1 <- mean(DataImp$AgeDistance1)
D2 <- mean(DataImp$AgeDistance2)
T1 <- mean(DataImp$AgeT1) 
T2 <- mean(DataImp$AgeT2) 
T3 <- mean(DataImp$AgeT3)

#Trajectory: (T2-T1) + (T3-T2)
DataImp$Trajectory <- as.numeric((DataImp$b2Scaled - DataImp$b1Scaled)*(D1/DataImp$AgeDistance1)+
                  (DataImp$b3Scaled - DataImp$b2Scaled)*(D2/DataImp$AgeDistance2))

#Height: T1 + T2 + T3
DataImp$Height <- as.numeric((DataImp$b1Scaled)*(T1/DataImp$AgeT1) +
                    (DataImp$b2Scaled)*(T2/DataImp$AgeT2) +
                    (DataImp$b3Scaled)*(T3/DataImp$AgeT3))
...
# Helmert coding
DataImp$SexOrthogonal <- as.factor(DataImp$Sex)
contrasts(DataImp$SexOrthogonal) <- contr.helmert(nlevels(DataImp$SexOrthogonal))

#"1" = "Male", "2" = "Female"

DataImp$ADIRaw <- DataImp$ADI
DataImp$ADI <- scale(DataImp$ADI)
```

# Elbow & Silhouette Method to determine number of clusters
```{r}
setwd("~/Library/CloudStorage/OneDrive-King'sCollegeLondon/Project 4/Code/Results & Graphics/K-Means/Optimal Nr of Clusters")
...
```{r}
setwd("~/Library/CloudStorage/OneDrive-King'sCollegeLondon/Project 4/Code/Results & Graphics/K-Means")
...
```{r}
setwd("~/Library/CloudStorage/OneDrive-King'sCollegeLondon/Project 4/Code/Results & Graphics/K-Means/MR Results/Tables")
...
## Regression results
```{r}
setwd("~/Library/CloudStorage/OneDrive-King'sCollegeLondon/Project 4/Code/Results & Graphics/K-Means/MR Results/Individual Results")
options(scipen = 999)
...
```{r}
setwd("~/Library/CloudStorage/OneDrive-King'sCollegeLondon/Project 4/Code/Results & Graphics/K-Means/MR Results")
